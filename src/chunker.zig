    ...
    while (i < content.len) {
        if (i + 3 <= content.len and std.mem.eql(u8, content[i .. i + 3], "```")) {
            if (!in_fence) {
                ...
                fences.append(std.heap.page_allocator, .{ .start = fence_start, .end = end, .lang = content[lang_start..i] }) catch {};
                in_fence = false;
                i += 3;
                continue;
            }
...
pub fn chunkDocument(content: []const u8) struct { chunks: std.ArrayList([]const u8) } {
    ...
    while (pos < content.len) {
        ...
        if (chunk_end >= content.len) {
            chunks.append(std.heap.page_allocator, content[pos..content.len]) catch {};
            break;
        }
        ...
        chunks.append(std.heap.page_allocator, content[pos..cutoff]) catch {};
        pos = cutoff;
    }
...
test "findCodeFences finds fenced blocks" {
    ...
    const result = findCodeFences(content);
